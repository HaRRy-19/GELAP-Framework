...
from pm4py.util import constants, exec_utils, pandas_utils, typing
...
def apply_log(log, petri_net, initial_marking, final_marking, parameters=None, variant=DEFAULT_VARIANT):
    ...
    # Debugging step: Print variants_idxs and all_alignments
    print(f"Total unique variants: {len(variants_idxs)}")
    for variant, indices in variants_idxs.items():
        print(f"Variant: {variant}, Original Indices: {indices}")
    ...
    # Debugging step: Print total alignments before re-expansion
    print(f"Total alignments created for unique variants: {len(all_alignments)}")
    
    alignments = __form_alignments(variants_idxs, all_alignments)
    
    # Debugging step: Check if re-expansion matches expected trace count
    print(f"Total aligned traces after re-expansion: {len(alignments)}")
    if len(alignments) == len(log):
        print("Re-expansion successful; all traces accounted for.")
    else:
        print(f"Mismatch in trace counts. Expected: {len(log)}, but got: {len(alignments)}")
...
def apply_multiprocessing(log, petri_net, initial_marking, final_marking, parameters=None, variant=DEFAULT_VARIANT):
    ...
    # Limit to a single core to reduce CPU usage
    num_cores = 1
...
def __form_alignments(variants_idxs, all_alignments):
    ...
    for index_variant, variant in enumerate(variants_idxs):
        for trace_idx in variants_idxs[variant]:
            # Map each original trace index to the aligned variant
            al_idx[trace_idx] = all_alignments[index_variant] 

    # Fill the final alignments list with all traces re-expanded
    alignments = []
    for i in range(len(al_idx)):
        alignments.append(al_idx[i])

    # If any original indices are missing, fill them with a placeholder or null alignment
    if len(alignments) < len(variants_idxs):
        missing_count = len(variants_idxs) - len(alignments)
        print(f"Missing {missing_count} alignments; adding placeholders.")
        alignments.extend([None] * missing_count)  # Or provide default alignment if appropriate
